...
Public Sub positionRCFormByMonitorSize()
    ...
    Dim volumeFormMonitorPrimary As Long: volumeFormMonitorPrimary = 0
    ...
    If gblMonitorCount > 1 And (LTrim$(gblMultiMonitorResize) = "1" Or LTrim$(gblMultiMonitorResize) = "2") Then
        ...
        ' note the monitor ID at volumeForm form_load and store as the volumeFormMonitorID
        gaugeMonitorStruct = cWidgetFormScreenProperties(fVolume.volumeForm, volumeFormMonitorID)
        
        volumeFormMonitorPrimary = gaugeMonitorStruct.IsPrimary
        
        If fVolume.volumeForm.Left = oldGaugeLeftPixels Then Exit Sub ' this can only work if the reposition is being performed by the timer
        ...
        If oldGaugeLeftPixels = 0 Then oldGaugeLeftPixels = fVolume.volumeForm.Left
    
        If gblOldvolumeFormMonitorPrimary <> volumeFormMonitorPrimary Then
            
            ' screenWrite ("Stored monitor primary status = " & CBool(gblOldvolumeFormMonitorPrimary))
            ' screenWrite ("Current monitor primary status = " & CBool(volumeFormMonitorPrimary))
            ...
            If LTrim$(gblMultiMonitorResize) = "1" Then
                ...
                If monitorStructWidthTwips <> oldMonitorStructWidthTwips Or monitorStructHeightTwips <> oldMonitorStructHeightTwips Then
                    ...
                    If fVolume.volumeForm.Left > oldGaugeLeftPixels Then
                        fVolume.volumeForm.Left = fVolume.volumeForm.Left + fVolume.volumeForm.Widgets("maincasingsurround").Widget.Left
                    Else
                        fVolume.volumeForm.Left = fVolume.volumeForm.Left - fVolume.volumeForm.Widgets("maincasingsurround").Widget.Left
                    End If
                    fVolume.volumeForm.Refresh
                    Call fVolume.AdjustZoom(resizeProportion)
                End If
            ElseIf LTrim$(gblMultiMonitorResize) = "2" Then
