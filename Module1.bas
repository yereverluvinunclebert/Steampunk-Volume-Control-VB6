...
Public gblOldvolumeFormMonitorPrimary As Long
Public gblPrefsFormResizedInCode As Boolean

Public gblfVolumeAvailable As Boolean
...
Public Function fLicenceState() As Integer
    ...
    If fFExists(gblSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\SteampunkVolumeControl", "licence", gblSettingsFile)
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hWnd, "Open", "https://github.com/yereverluvinunclebert/Steampunk-Volume-Control-" & gblCodingEnvironment & "/issues", vbNullString, App.Path, 1)
    End If
...
Public Sub mainScreen()
   On Error GoTo mainScreen_Error
    ...
    If fVolume.volumeForm.Left > gblVirtualScreenWidthPixels - 50 Then
        fVolume.volumeForm.Left = gblVirtualScreenWidthPixels - 150
    End If
    If fVolume.volumeForm.Top > gblVirtualScreenHeightPixels - 50 Then
        fVolume.volumeForm.Top = gblVirtualScreenHeightPixels - 150
    End If
'
    ' calculate the current hlocation in % of the screen
    ' store the current hlocation in % of the screen
    If gblWidgetPosition = "1" Then
        gblhLocationPercPrefValue = CStr(fVolume.volumeForm.Left / gblVirtualScreenWidthPixels * 100)
        gblvLocationPercPrefValue = CStr(fVolume.volumeForm.Top / gblVirtualScreenHeightPixels * 100)
    End If
...
Public Sub reloadProgram()
    ...
    'fVolume.ShowHelp = False ' needs to be set to false for the reload to reshow it, if enabled
    
    gblfVolumeAvailable = False ' tell the ' screenWrite util that the volumeForm is no longer available to write console events to
...
Public Sub saveMainRCFormPosition()
   ...
   On Error GoTo saveMainRCFormPosition_Error
    ...
    If gblDpiAwareness = "1" Then
        gblGaugeHighDpiXPos = CStr(fVolume.volumeForm.Left) ' saving in pixels
        gblGaugeHighDpiYPos = CStr(fVolume.volumeForm.Top)
    ...
    Else
        gblGaugeLowDpiXPos = CStr(fVolume.volumeForm.Left) ' saving in pixels
        gblGaugeLowDpiYPos = CStr(fVolume.volumeForm.Top)
    ...
    gblGaugeSize = CStr(fVolume.volumeForm.WidgetRoot.Zoom * 100)
...
Public Sub readPrefsPosition()
    ...
    If gblDpiAwareness = "1" Then
        gblPrefsHighDpiXPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formHighDpiXPosTwips", gblSettingsFile)
        gblPrefsHighDpiYPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formHighDpiYPosTwips", gblSettingsFile)
    ...
    Else
        gblPrefsLowDpiXPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formLowDpiXPosTwips", gblSettingsFile)
        gblPrefsLowDpiYPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPositionAndSize()
    ...
    If widgetPrefs.IsLoaded = True And widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\SteampunkVolumeControl", "formHighDpiXPosTwips", gblPrefsHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\SteampunkVolumeControl", "formHighDpiYPosTwips", gblPrefsHighDpiYPosTwips, gblSettingsFile
        Else
            gblPrefsLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblPrefsLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\SteampunkVolumeControl", "formLowDpiXPosTwips", gblPrefsLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\SteampunkVolumeControl", "formLowDpiYPosTwips", gblPrefsLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hWnd, "open", thisCommand, "Steampunk-Volume-Control-VB6.exe prefs", "", 1)
    Else
...
'    fVolume.volumeForm.Widgets("busy1").Widget.Alpha = 0
'    fVolume.volumeForm.Widgets("busy2").Widget.Alpha = 0
'    fVolume.volumeForm.Widgets("busy3").Widget.Alpha = 0
'    fVolume.volumeForm.Widgets("busy4").Widget.Alpha = 0
'    fVolume.volumeForm.Widgets("busy5").Widget.Alpha = 0
'    fVolume.volumeForm.Widgets("busy6").Widget.Alpha = 0
'
'    fVolume.volumeForm.Widgets("busy1").Widget.Refresh
'    fVolume.volumeForm.Widgets("busy2").Widget.Refresh
'    fVolume.volumeForm.Widgets("busy3").Widget.Refresh
'    fVolume.volumeForm.Widgets("busy4").Widget.Refresh
'    fVolume.volumeForm.Widgets("busy5").Widget.Refresh
'    fVolume.volumeForm.Widgets("busy6").Widget.Refresh
