...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hwnd, "Open", "https://github.com/yereverluvinunclebert/Steampunk-Volume-Control-VB6/issues", vbNullString, App.path, 1)
    End If
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If gblDpiAwareness = "1" Then
        gblFormHighDpiXPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formHighDpiXPosTwips", gblSettingsFile)
        gblFormHighDpiYPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formHighDpiYPosTwips", gblSettingsFile)
...
'        End If
    Else
        gblFormLowDpiXPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formLowDpiXPosTwips", gblSettingsFile)
        gblFormLowDpiYPosTwips = fGetINISetting("Software\SteampunkVolumeControl", "formLowDpiYPosTwips", gblSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        ...
        Else
            ...
            sPutINISetting "Software\SteampunkVolumeControl", "formLowDpiYPosTwips", gblFormLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hwnd, "open", thisCommand, "Steampunk Volume Control.exe prefs", "", 1)
    Else
