...
Public Sub mainRoutine(ByVal restart As Boolean)
    ...
    ' = "none"
    gblStartupFlg = True
    gblWidgetName = "Steampunk Volume Control"
    thisPSDFullPath = App.Path & "\Res\SteampunkVolumeMerged.psd"
    ...
    menuForm.mnuAbout.Caption = "About Steampunk Volume Control Cairo " & gblCodingEnvironment & " widget"
    ...
    Call readSettingsFile("Software\SteampunkVolumeControl", gblSettingsFile)
...
' Author    : beededea
' Date      : 20/02/2025
' Purpose   : note the monitor primary at the main form_load and store as gblOldvolumeFormMonitorPrimary - will be resampled regularly later and compared
'---------------------------------------------------------------------------------------
'
Private Sub identifyPrimaryMonitor()
    Dim volumeFormMonitorID As Long: volumeFormMonitorID = 0
    
    On Error GoTo identifyPrimaryMonitor_Error

    gaugeMonitorStruct = cWidgetFormScreenProperties(fVolume.volumeForm, volumeFormMonitorID)
    gblOldvolumeFormMonitorPrimary = gaugeMonitorStruct.IsPrimary
...
Private Sub checkFirstTime()
   ...
   On Error GoTo checkFirstTime_Error
    ...
    If gblFirstTimeRun = "true" Then
        ...
        sPutINISetting "Software\SteampunkVolumeControl", "firstTimeRun", gblFirstTimeRun, gblSettingsFile
    End If
...
'    add Resources to the global ImageList that are not being pulled from the PSD directly
    ...
    Cairo.ImageList.AddImage "help", App.Path & "\Resources\images\Steampunk-volume-help.png"
    ...
    Cairo.ImageList.AddImage "general-icon-dark-clicked", App.Path & "\Resources\images\general-icon-dark-1010-clicked.jpg"
    ...
    Cairo.ImageList.AddImage "general-icon-light-clicked", App.Path & "\Resources\images\general-icon-light-1010-clicked.jpg"
   ...
   On Error GoTo adjustMainControls_Error
    ...
    If gblMonitorCount > 1 And (LTrim$(gblMultiMonitorResize) = "1" Or LTrim$(gblMultiMonitorResize) = "2") Then
        If gaugeMonitorStruct.IsPrimary = True Then
            Call fVolume.AdjustZoom(Val(gblGaugePrimaryHeightRatio))
        Else
            Call fVolume.AdjustZoom(Val(gblGaugeSecondaryHeightRatio))
        End If
    Else
...
'    End With
   ...
   ' set the lock state of the gauge
    ...
    overlayWidget.opacity = Val(gblOpacity)
...
'        gblVolumeBoost = fGetINISetting(Location, "volumeBoost", gblSettingsFile)
        ...
        ' other
        gblGaugeHighDpiXPos = fGetINISetting("Software\SteampunkVolumeControl", "gaugeHighDpiXPos", gblSettingsFile)
        gblGaugeHighDpiYPos = fGetINISetting("Software\SteampunkVolumeControl", "gaugeHighDpiYPos", gblSettingsFile)
        gblGaugeLowDpiXPos = fGetINISetting("Software\SteampunkVolumeControl", "gaugeLowDpiXPos", gblSettingsFile)
        gblGaugeLowDpiYPos = fGetINISetting("Software\SteampunkVolumeControl", "gaugeLowDpiYPos", gblSettingsFile)
...
Private Sub getToolSettingsFile()
    ...
    gblSettingsDir = fSpecialFolder(feUserAppData) & "\SteampunkVolumeControl" ' just for this user alone
...
Private Sub handleUnhideMode(ByVal thisUnhideMode As String)
    ...
    If thisUnhideMode = "unhide" Then     'parse the command line
        gblUnhide = "true"
        sPutINISetting "Software\SteampunkVolumeControl", "unhide", gblUnhide, gblSettingsFile
